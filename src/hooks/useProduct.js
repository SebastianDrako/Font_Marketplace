...
import { getProductById, getCategoryById, getProductsByCategory } from '../services/productService';
    ...
    const fetchAllData = async () => {
      ...
      try {
        ...
        if (productData.categoryId) {
          ...
          while (currentCategoryId && currentCategoryId !== 0 && !visitedIds.has(currentCategoryId)) {
        ...
        console.log('[useProduct] 3. Iniciando fetch de productos relacionados.');
        ...
        for (const category of categorySearchOrder) {
          ...
          const products = await getProductsByCategory(category.id, 0, 7); // Pedimos 7 por si viene el producto actual
        ...
        console.log(`[useProduct] Productos relacionados encontrados: ${collectedProducts.length}`, collectedProducts);
